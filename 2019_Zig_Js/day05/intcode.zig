...
pub fn Computer(comptime max_input: usize, comptime max_mem: usize) type {
    return struct {
        ...
        pub fn init(allocator: std.mem.Allocator, code: []const i64) Self {
            var self: Self = undefined;
            @memset(&self.mem, 0);
            @memcpy(self.mem[0..code.len], code);
            self.pc = 0;
            self.rel_base = 0;
            self.input_queue = std.ArrayList(i64).initCapacity(allocator, max_input) catch unreachable;
        ...
        pub fn reset(self: *Self, code: []const i64) void {
            @memset(&self.mem, 0);
            @memcpy(self.mem[0..code.len], code);
            self.pc = 0;
            self.rel_base = 0;
        ...
        pub fn addInput(self: *Self, val: i64) void {
            self.input_queue.append(self.allocator, val) catch unreachable;
        }
            ...
            while (!self.halted) {
                ...
                switch (opcode) {
                    ...
                    3 => {
                        ...
                        const val = self.input_queue.items[0];
